...
export const userService = {
	createUser: async (
		user: CreateUser
	): Promise<InternalServiceResponse<User | null>> => {
		try {
			...
			return new InternalServiceResponse<User>(
	...
	checkEmailExists: async (
		email: string
	): Promise<InternalServiceResponse<boolean>> => {
		try {
			const user = await userRepository.findByEmail(email);
			if (!user) {
				return new InternalServiceResponse<boolean>(
