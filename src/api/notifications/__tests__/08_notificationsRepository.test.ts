...
describe('notificationRepository', () => {
	...
	beforeAll(async () => {
		...
		await Promise.all([
			EntityFactory.createNotification(trx, 1, 1, 3, 'invite'),
			EntityFactory.createNotification(trx, 2, 1, 2, 'newMessage', true),
			EntityFactory.createNotification(trx, 3, 2, 3, 'invite'),
			EntityFactory.createNotification(trx, 4, 2, 4, 'invite'),
		]);
	});
	...
	test('createNotification', async () => {
		...
		await notificationsRepository.deleteNotification(trx, notification.id);
	});

	test('getNotificationByRecipientId', async () => {
		const notifications = await notificationsRepository.getNotificationsByRecipientId(trx, 2);
		expect(notifications).toHaveLength(2);
	});

	test('getUnreadNotificationsByRecipientId', async () => {
		const notifications = await notificationsRepository.getUnreadNotificationsByRecipientId(trx, 1);
		expect(notifications).toHaveLength(1);
	});

	test('markAsRead', async () => {
		await notificationsRepository.markAsRead(trx, 1);
		const notification = await trx('notifications').where('id', 1).first();
	...
	afterAll(async () => {
		await EntityFactory.deleteNotifications(trx, [2, 3, 4]);
		await trx.schema.alterTable('notifications', (table: any) => {
